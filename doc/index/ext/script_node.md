...
## 使用脚本的处理逻辑节点<sup class="footnote-symbol"> <font color=orange>[115]</font></sup>
...
def reload_tag = "reloadsignal-$system_id-sysaifactory-ai-ibizintelligence"
def config = [:]
...
```
#### [智能体业务上下文(AI_AGENT_CONTEXT)](module/ai/ai_agent_context)的处理逻辑[agent_flow_templ](module/ai/ai_agent_context/logic/agent_flow_templ)
...
```
#### [智能体业务上下文(AI_AGENT_CONTEXT)](module/ai/ai_agent_context)的处理逻辑[交谈全文内容推理(chat_fulltext_reason)](module/ai/ai_agent_context/logic/chat_fulltext_reason)
...
```
#### [智能体业务上下文(AI_AGENT_CONTEXT)](module/ai/ai_agent_context)的处理逻辑[批量执行(batch_execution)](module/ai/ai_agent_context/logic/batch_execution)
...
```
#### [智能体业务上下文(AI_AGENT_CONTEXT)](module/ai/ai_agent_context)的处理逻辑[查表审查(lookup)](module/ai/ai_agent_context/logic/lookup)
...
```
#### [智能体业务上下文(AI_AGENT_CONTEXT)](module/ai/ai_agent_context)的处理逻辑[深度研究(deep_research)](module/ai/ai_agent_context/logic/deep_research)
...
```
#### [智能体业务上下文(AI_AGENT_CONTEXT)](module/ai/ai_agent_context)的处理逻辑[辅助生成引导提示词（停用）(guided_prompt)](module/ai/ai_agent_context/logic/guided_prompt)
...
```
#### [智能体记忆任务实例(AI_AGENT_MEMORY_TASK)](module/ai/ai_agent_memory_task)的处理逻辑[保存记忆分块(save_chunk)](module/ai/ai_agent_memory_task/logic/save_chunk)
...
```
#### [智能体记忆任务实例(AI_AGENT_MEMORY_TASK)](module/ai/ai_agent_memory_task)的处理逻辑[填充默认文档标识(fill_default_doc_id)](module/ai/ai_agent_memory_task/logic/fill_default_doc_id)
...
```
#### [智能体记忆任务实例(AI_AGENT_MEMORY_TASK)](module/ai/ai_agent_memory_task)的处理逻辑[更新每日记忆文档(update_daily_log)](module/ai/ai_agent_memory_task/logic/update_daily_log)
...
```
#### [智能体记忆任务实例(AI_AGENT_MEMORY_TASK)](module/ai/ai_agent_memory_task)的处理逻辑[获取记忆分块(get_chunk)](module/ai/ai_agent_memory_task/logic/get_chunk)
...
```
#### [智能体记忆任务实例(AI_AGENT_MEMORY_TASK)](module/ai/ai_agent_memory_task)的处理逻辑[获取记忆文档(get_document)](module/ai/ai_agent_memory_task/logic/get_document)
...
```
#### [智能体记忆任务实例(AI_AGENT_MEMORY_TASK)](module/ai/ai_agent_memory_task)的处理逻辑[记忆提取并存储(extract_and_store)](module/ai/ai_agent_memory_task/logic/extract_and_store)
...
```
#### [AI客户端凭证(AI_CLIENT_CREDENTIAL)](module/ai/ai_client_credential)的处理逻辑[exrouter](module/ai/ai_client_credential/logic/exrouter)
...
```
#### [知识库文档分块(AI_KB_CHUNK)](module/ai/ai_kb_chunk)的处理逻辑[获取pageIndex信息(get_page_index_info)](module/ai/ai_kb_chunk/logic/get_page_index_info)
...
```
#### [知识库文档(AI_KB_DOCUMENT)](module/ai/ai_kb_document)的处理逻辑[全文推理(reason)](module/ai/ai_kb_document/logic/reason)
...
```
#### [知识库文档(AI_KB_DOCUMENT)](module/ai/ai_kb_document)的处理逻辑[获取fullText信息(get_full_text_info)](module/ai/ai_kb_document/logic/get_full_text_info)
...
```
#### [知识库文档(AI_KB_DOCUMENT)](module/ai/ai_kb_document)的处理逻辑[获取pageIndex信息(get_page_index_info)](module/ai/ai_kb_document/logic/get_page_index_info)
...
```
#### [知识库文档同步(AI_KB_DOCUMENT_SYNC)](module/ai/ai_kb_document_sync)的处理逻辑[空间文档解析处理(space_parsing)](module/ai/ai_kb_document_sync/logic/space_parsing)
...
if(import_method == "space_manual"){
    ...
    if(selection_page_ids){
        ...
        selection_page_ids.split(',').each { String page_id ->
            ...
            if(is_parse_now == 1){
                println("立即解析文档...")
                new_doc.set("status",2)
                doc_runtime.create(new_doc)
                doc_runtime.Async_reparse(new_doc)
            }else{
                println("不解析...")
                new_doc.set("status",0)
                doc_runtime.create(new_doc)
    ...
    if(selection_file_ids){
        ...
        files.each { file ->
            ...
            new_doc.set("active",1)
            new_doc.set("kb_id",kb_id)
            new_doc.set("categories",file_path)

              //立即解析
            if(is_parse_now == 1){
                println("立即解析上传文档...")
                new_doc.set("status",3)
                doc_runtime.create(new_doc)
            }
            else {
                new_doc.set("status",0)
...
```
#### [知识库(AI_KNOWLEDGE_BASE)](module/ai/ai_knowledge_base)的处理逻辑[all_doc_reason](module/ai/ai_knowledge_base/logic/all_doc_reason)
...
```
#### [知识库(AI_KNOWLEDGE_BASE)](module/ai/ai_knowledge_base)的处理逻辑[get_by_code](module/ai/ai_knowledge_base/logic/get_by_code)
...
```
#### [知识库(AI_KNOWLEDGE_BASE)](module/ai/ai_knowledge_base)的处理逻辑[keywords计算(keywords)](module/ai/ai_knowledge_base/logic/keywords)
...
```
#### [知识库(AI_KNOWLEDGE_BASE)](module/ai/ai_knowledge_base)的处理逻辑[全文内容推理(fulltext_reason)](module/ai/ai_knowledge_base/logic/fulltext_reason)
...
```
#### [知识库(AI_KNOWLEDGE_BASE)](module/ai/ai_knowledge_base)的处理逻辑[推理(reason)](module/ai/ai_knowledge_base/logic/reason)
...
```
#### [知识库(AI_KNOWLEDGE_BASE)](module/ai/ai_knowledge_base)的处理逻辑[深度研究(deep_research)](module/ai/ai_knowledge_base/logic/deep_research)
...
```
#### [知识库(AI_KNOWLEDGE_BASE)](module/ai/ai_knowledge_base)的处理逻辑[知识库切换（对话窗口）(switch_set)](module/ai/ai_knowledge_base/logic/switch_set)
...
```
#### [知识库(AI_KNOWLEDGE_BASE)](module/ai/ai_knowledge_base)的处理逻辑[获取summary信息(get_summary)](module/ai/ai_knowledge_base/logic/get_summary)
...
```
#### [AI大模型(AI_MODEL)](module/ai/ai_model)的处理逻辑[获取模型提供商版本(provider_model_version)](module/ai/ai_model/logic/provider_model_version)
...
```
#### [智能审查报告(AI_REVIEW_REPORT)](module/ai/ai_review_report)的处理逻辑[upsert](module/ai/ai_review_report/logic/upsert)
...
```
#### [智能审查报告(AI_REVIEW_REPORT)](module/ai/ai_review_report)的处理逻辑[获取转换html(ConvertedHTML)](module/ai/ai_review_report/logic/ConvertedHTML)
...
```
#### [类别设置(CATEGORY_SETTINGS)](module/Base/category_settings)的处理逻辑[get_aifactory_sys_env](module/Base/category_settings/logic/get_aifactory_sys_env)
...
```
#### [类别设置(CATEGORY_SETTINGS)](module/Base/category_settings)的处理逻辑[save_aifactory_sys_env](module/Base/category_settings/logic/save_aifactory_sys_env)
...
var viewPortlets = portletSearchContext.in("psappportletid",biPortletIds.join(",")).eq("pssysappid","web").ne("dynamodelflag",0).pageable(0,200).select();
