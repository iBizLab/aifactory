## 无父页面(no_parent_page) <!-- {docsify-ignore-all} -->
...
(`PARENT_ID(父页面标识)` ISNULL AND `ID(标识)` NOTEQ `网页请求上下文.n_space_id_eq` AND `IS_ARCHIVED(是否已归档)` EQ `'0'` AND `IS_DELETED(是否已删除)` EQ `'0'` AND `IS_PUBLISHED(是否发布)` EQ `'1'`)
