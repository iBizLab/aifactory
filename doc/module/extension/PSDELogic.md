...
|启用|VALIDFLAG|是否逻辑||是||
|WebHook调试参数|WEBHOOKDEBUGPARAMS|长文本，没有长度限制|1048576|是||
...
| --------| --------| -------- |------------|------------|------------|
| 直接应用扩展模型 | apply2 | 应用 |单项数据（主键）|<details><summary>后台调用</summary>[apply](#行为)||
| 新建计划规则 | create_schedule_flow | 计划规则 |无数据|<details><summary>打开视图或向导（模态）</summary>[新建规则](app/view/psde_logic_quick_create_view_schedule)</details>||
| 打开配置视图 | open_quick_cfg_view | 编辑 |单项数据（主键）|<details><summary>打开视图或向导（模态）</summary>[自动化规则](app/view/psde_logic_quick_cfg_view)</details>||
| 执行webhook调试 | debug_webhook | 执行调试 |单项数据|<details><summary></summary></details>||
| 应用扩展模型 | apply | 应用 |单项数据（主键）|<details><summary>后台调用</summary>[apply](#行为)||
| 新建自动化规则 | create_auto_flow | 自动化规则 |无数据|<details><summary>打开视图或向导（模态）</summary>[新建规则](app/view/psde_logic_quick_create_view_auto_flow)</details>||
| 打开webhook调试视图 | open_webhook_debug_view | webhook调试 |单项数据|<details><summary>打开视图或向导（模态）</summary>[WebHook调试](app/view/psde_logic_web_hook_debug_view)</details>||
| 保存调试参数 | update_debug_params | 保存调试参数 |单项数据|<details><summary>后台调用</summary>[Update](#行为)||
...
<el-anchor-link :href="`#/module/extension/PSDELogic?id=界面逻辑`">
  界面逻辑
</el-anchor-link>
</el-anchor>
</div>
