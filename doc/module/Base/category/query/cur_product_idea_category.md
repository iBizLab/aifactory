## 当前产品需求类别(cur_product_idea_category) <!-- {docsify-ignore-all} -->
...
(`OWNER_TYPE(所属数据对象)` EQ `'product'` AND `OWNER_SUBTYPE(所属对象子类型)` EQ `'idea'` AND `OWNER_ID(所属数据标识)` EQ `数据上下文.product`)
