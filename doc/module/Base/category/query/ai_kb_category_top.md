## 知识库目录（顶级）(ai_kb_category_top) <!-- {docsify-ignore-all} -->
...
(`OWNER_TYPE(所属数据对象)` EQ `'ai_knowledge_base'` AND `OWNER_SUBTYPE(所属对象子类型)` EQ `'ai_knowledge_base'` AND `PID(父标识)` ISNULL)
