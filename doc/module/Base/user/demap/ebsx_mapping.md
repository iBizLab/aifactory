...
* `属性等价`
`CREATE_TIME(建立时间)` <i class="fa fa-angle-double-right"/></i> `CREATEDATE(创建时间)`
* `属性等价`
...
* `属性等价`
`UPDATE_TIME(更新时间)` <i class="fa fa-angle-double-right"/></i> `UPDATEDATE(最后修改时间)`
* `属性等价`
...
* `默认`
`重置密码` <i class="fa fa-angle-double-right"/></i> `初始化密码`
* `默认`
